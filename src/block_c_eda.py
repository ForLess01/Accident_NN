...
def write_interpretations(metrics: dict[str, object]) -> None:
    lines = [
        ...
        f"H8: Las condiciones pre-impacto separan el riesgo (fig11): zona rural {metrics['rural_rate']:.1f}% vs urbana {metrics['urban_rate']:.1f}%, y clima lluvioso {metrics['rain_rate']:.1f}% vs despejado {metrics['clear_rate']:.1f}%. Estas variables no existían en el dataset SUTRAN y son el aporte central del cambio de fuente.",
