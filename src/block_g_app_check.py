...
from src.app_inference import load_demo_cases, predict_records
...
def run_block_g_check() -> dict[str, object]:
    ...
    checklist = [
        ...
        {"check": "response_under_2_seconds", "passed": elapsed < 2.0, "detail": f"{elapsed:.3f}s"},
        {"check": "atropello_changes_probability", "passed": float(atropello) != float(base), "detail": f"base={base:.4f}; atropello={atropello:.4f}"},
        {"check": "unseen_road_code_does_not_break", "passed": pd.notna(unseen), "detail": f"probabilidad={unseen:.4f}"},
        {"check": "puno_case_does_not_break", "passed": pd.notna(puno), "detail": f"probabilidad={puno:.4f}"},
    ]
